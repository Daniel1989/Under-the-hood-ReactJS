...
<em>1.0 第一部分（点击查看大图）</em>

### 事务

组件应该以某种方式**接入并影响**到React的生态系统里。有一个`ReactUpdates`模块，主要目的就是用来完成此功能的。正如大家所知悉的，**React是成块更新的**,也就是说，它会把所有操作**聚合**到一起。这样做的好处是，它可以一次性检测所有操作的先决条件和后决条件，而不需要每个操作单独去检测这些。
